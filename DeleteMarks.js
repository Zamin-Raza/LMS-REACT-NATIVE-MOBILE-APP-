...
const DeleteMarks = ({ navigation, route }) => {
      ...
      if (marksDoc.exists) {
        ...
        if (examData && examData[selectedSubject]) {
          ...
          if (subjectData) {
            ...
            setMarks(subjectData.marks.toString());
          } else {
  ...
  const deleteMarks = async () => {
    if (!selectedExam || !selectedSubject || !selectedStudent) {
      Alert.alert('Please select exam type, subject, and student');
      return;
    }
    ...
    try {
      ...
      const marksData = {
        [selectedExam]: {
          [selectedSubject]: {
            [selectedStudent]: firestore.FieldValue.delete(),
            marks: firestore.FieldValue.delete(),
          },
        },
      };
  
      await marksRef.set(marksData, { merge: true });
      Alert.alert('Marks deleted successfully');
    } catch (error) {
      console.error('Error deleting marks:', error);
      Alert.alert('Error deleting marks');
    }
  };
